		...
		try {
			...
			if (identifying == 0) {
				...
				Query query = em.createNativeQuery("select *"
						+ " from rbac_user user where user.user_code = :userCode and user.user_password = :userPass and user.del_flag = 0", RbacUser.class);
				...
				// 用户信息查询
				Query query = em.createNativeQuery("select *"
						+ " from papi_token where api_code = :userCode and api_password = :userPass and del_flag = 0", PapiToken.class);
				query.setParameter("userCode", authentication.getPrincipal());
