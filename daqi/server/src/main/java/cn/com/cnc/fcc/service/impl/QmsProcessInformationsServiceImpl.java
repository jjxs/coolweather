...
public class QmsProcessInformationsServiceImpl implements QmsProcessInformationsService {
	...
	private final Logger log = LoggerFactory.getLogger(QmsProcessInformationsServiceImpl.class);
	...
	public QmsProcessInformationsServiceImpl(EntityManagerFactory emf,
			QmsBomTechnologyRepository qmsBomTechnologyRepository,
			QmsQualityControlDetailsRepository qmsQualityControlDetailsRepository,
			QmsPartsAssemblyRelationRepository qmsPartsAssemblyRelationRepository,
			QmsEnclosureRepository qmsEnclosureRepository,
			QmsProcessRepository qmsProcessRepository
			){
		this.emf = emf;
	...
	public List<QmsOrganizationInfoDTO> getParentNodeListInfo(String materielID, String vehicleId) {
		...
		try {
			...
			// 赋值给DTO
			qmsOrganizationInfoDTO = query.unwrap( org.hibernate.query.NativeQuery.class )
					.addScalar("id",StandardBasicTypes.BIG_INTEGER)
					.addScalar("parentCd",StandardBasicTypes.STRING)
					.addScalar("organizationCd",StandardBasicTypes.STRING)
					.addScalar("organizationName",StandardBasicTypes.STRING)
					.addScalar("vehicleType",StandardBasicTypes.STRING)
					.addScalar("materielCd",StandardBasicTypes.STRING)
	...
	public String getSelectAllInfo(){
		...
		StringBuffer objSqlContent = new StringBuffer();
		objSqlContent.append(" WITH RECURSIVE sub1 (id,materiel_id,materiel_name,materiel_cd,vehicleType,parent_materiel_id) AS ( ");
	...
	public List<ProcessInfomationsRightListDTO> selectAllInfo(HashMap<String, Object> param) {
		...
		try {
			...
			processInfomationsRightList = query.unwrap( org.hibernate.query.NativeQuery.class )
					.addScalar("id",StandardBasicTypes.LONG)
					.addScalar("orderNo",StandardBasicTypes.STRING)
					.addScalar("processCd",StandardBasicTypes.STRING)
					.addScalar("processName",StandardBasicTypes.STRING)
					.setResultTransformer(transformer)
					.getResultList();
			query = null;
		} catch (Exception e) {
	...
	public List<ProcessInfomationsRightListDTO> getAllInfoNumber(HashMap<String, Object> param) {
		...
		try {
			...
			processInfomationsRightList = query.unwrap( org.hibernate.query.NativeQuery.class )
					.addScalar("numberCount",StandardBasicTypes.STRING)
					.setResultTransformer(transformer)
					.getResultList();
			query = null;
		} catch (Exception e) {
	...
	public List<DropDowmValueDTO> getCarTypeInfo() {
		...
		try{
			...
			dropDowmValueDTO = query.unwrap( org.hibernate.query.NativeQuery.class )
	...
	public Integer createInfo(BomTechnologGroupDTO bomTechnologGroupDTO) {
		...
		try {
			// 根据物料和工艺编码取值
			List<QmsBomTechnology> existenceCheckQBT = qmsBomTechnologyRepository.findByMaterielIdAndTechnologyCdAndOrderNoAndFlagStatus(bomTechnologGroupDTO.getQmsBomTechnology().getMaterielId(),
					bomTechnologGroupDTO.getQmsBomTechnology().getTechnologyCd(),bomTechnologGroupDTO.getQmsBomTechnology().getOrderNo(), "0");
			// 判断物料和工艺编码是否已存在
	...
	public Integer updateInfo(BomTechnologGroupDTO bomTechnologGroupDTO) {
		...
		try {
			// 根据物料和工艺编码、工序号取值
			List<QmsBomTechnology> existenceCheckQBT = qmsBomTechnologyRepository.findByMaterielIdAndTechnologyCdAndOrderNoAndFlagStatusAndIdNot(bomTechnologGroupDTO.getQmsBomTechnology().getMaterielId(),
					bomTechnologGroupDTO.getQmsBomTechnology().getTechnologyCd(),bomTechnologGroupDTO.getQmsBomTechnology().getOrderNo(), "0",bomTechnologGroupDTO.getQmsBomTechnology().getId());
			// 判断物料和工艺编码、工序号是否已存在
			if(existenceCheckQBT.size() != 0){
				// 返回结果 物料和工艺编码已存在
				backResult=1;
	...
	public List<QmsPartsAssemblyRelationOwnerDTO> getFlagStatusAndBomTechnologyId(String FlagStatus, Integer BomTechnologyId) {
		...
		try {
			...
			QmsPartsAssemblyRelation = query.unwrap( org.hibernate.query.NativeQuery.class )
					.addScalar("id",StandardBasicTypes.LONG)
					.addScalar("bomTechnologyId",StandardBasicTypes.INTEGER)
					.addScalar("assemblyNum",StandardBasicTypes.INTEGER)
					.addScalar("assemblyMaterielId",StandardBasicTypes.INTEGER)
					.addScalar("assemblyMaterielCd",StandardBasicTypes.STRING)
					.addScalar("assemblyMaterielName",StandardBasicTypes.STRING)
					.addScalar("assemblyCount",StandardBasicTypes.INTEGER)
					.addScalar("remark",StandardBasicTypes.STRING)
					.addScalar("flagStatus",StandardBasicTypes.STRING)
					.addScalar("compPkid",StandardBasicTypes.STRING)
					.addScalar("reserveFirst",StandardBasicTypes.STRING)
					.addScalar("reserveSecond",StandardBasicTypes.STRING)
					.addScalar("reserveThird",StandardBasicTypes.STRING)
					.addScalar("makeUser",StandardBasicTypes.STRING)
					.addScalar("modifyUser",StandardBasicTypes.STRING)
					.setResultTransformer(transformer)
					.getResultList();
			query = null;
		} catch (Exception e) {
	...
	@Override
	@Transactional
	public HashMap<String,Object> createCopyProcessInfo(Integer hiddenRightMaterielId, String technologyCd, String technologyName,
			String copyTechnologyCd) {
