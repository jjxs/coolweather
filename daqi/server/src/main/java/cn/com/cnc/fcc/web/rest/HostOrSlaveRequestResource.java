...
public class HostOrSlaveRequestResource {
	...
	private final Logger log = LoggerFactory.getLogger(HostOrSlaveRequestResource.class);
	...
	private static final String HST_ERROR = "Cannot execute to Application. Reason: %s";
	
	public HostOrSlaveRequestResource(RbacUserRepository rbacUserRepository, HstServerInfoRepository hstServerInfoRepository,
			DateUtil dateUtil, HstServerInfoDetailsRepository hstServerInfoDetailsRepository) {
	 ...
	 * @return
	 */
	private Long saveHstServer(List<HstServerInfo> hstServerInfos, String localUrl, String localNode, ZonedDateTime nowDate, 
			String hostNodeInfo) {
	...
	/**
	 * @param hstServerInfoDetailss
	 * @param infoId
	 * @param nowDate
	 * @param isLoaclCheck
	 * @return
	 */
	private String saveHstServerDetails(List<HstServerInfoDetails> hstServerInfoDetailss, 
			Long infoId, ZonedDateTime nowDate, List<HstServerInfoDetails> postHstServerInfoDetailss) {
		...
		mapInfoDetails.put(infoId + 
				hostName + ipAddress + 
				osUser + osName + 
				osVersion + osArch, 1);
		
		for(HstServerInfoDetails hstServerInfoDetails: hstServerInfoDetailss) {
			if (!mapInfoDetails.containsKey(hstServerInfoDetails.getInfoId().toString() + 
					hstServerInfoDetails.getHostName().toString() + hstServerInfoDetails.getIpAddress().toString() + 
					hstServerInfoDetails.getOsUser().toString() + hstServerInfoDetails.getOsName().toString() + 
					hstServerInfoDetails.getOsVersion().toString() + hstServerInfoDetails.getOsArch().toString())) {
				mapInfoDetails.put(hstServerInfoDetails.getInfoId().toString() + 
						hstServerInfoDetails.getHostName().toString() + hstServerInfoDetails.getIpAddress().toString() + 
						hstServerInfoDetails.getOsUser().toString() + hstServerInfoDetails.getOsName().toString() + 
						hstServerInfoDetails.getOsVersion().toString() + hstServerInfoDetails.getOsArch().toString(), 1);
			}
		}
	 ...
	 * @return
	 */
	public String saveInfomation(List<HstServer> hstServers, String localUrl, 
			String localNode, String hostNodeInfo, List<HstServer> postHstServers) {
...
//		}
		...
		if (postHstServers != null) {
			...
			retMessage = this.saveHstServerDetails(hstServerInfoDetailss, infoId, nowDate, 
					postHstServerInfoDetailss);	
		} else {
			retMessage = this.saveHstServerDetails(hstServerInfoDetailss, infoId, nowDate, 
					null);	
		}
    ...
    @Timed
	public List<HstServer> getHostServerInfomationAndDetails(@RequestBody List<HstServer> hstServers) {
		...
		this.saveInfomation(localHstServer, hstServers.get(0).getHstServerInfo().getNodeUrl(), 
				hstServers.get(0).getHstServerInfo().getNodeId(), hstServers.get(0).getHstServerInfo().getpNodeId(), hstServers);
