...
public class QmsMaterielTypeServiceImpl implements QmsMaterielTypeService {
	...
	private final Logger log = LoggerFactory.getLogger(QmsMaterielTypeServiceImpl.class);
	...
	/**
	 * 取得父节点数据
	 * @param organizationCd 物料分类ID
	 * @param organizationName 物料分类名称
	 * @return
	 */
	...
	public Integer deleteNodeInfos(String id) {
		...
		try {
			...
			organizationInfo = query.unwrap( org.hibernate.query.NativeQuery.class )
					...
					.addScalar("parentCd",StandardBasicTypes.STRING)
					.addScalar("materielTypeCd",StandardBasicTypes.STRING)
					.addScalar("materielTypeName",StandardBasicTypes.STRING)
					.setResultTransformer(transformer)
					.getResultList();
			query = null;
    ...
    String getfindAllSql(String bianMa, String gongName) {
