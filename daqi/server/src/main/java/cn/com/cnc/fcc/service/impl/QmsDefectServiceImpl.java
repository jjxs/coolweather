...
public class QmsDefectServiceImpl implements QmsDefectService{
	...
	private final Logger log = LoggerFactory.getLogger(QmsDefectServiceImpl.class);
	...
	public Integer deleteNodeInfos(String id) {
		...
		try {
			...
			organizationInfoCheck = query1.unwrap( org.hibernate.query.NativeQuery.class )
					...
					.addScalar("parentCd",StandardBasicTypes.STRING)
					.addScalar("defectCd",StandardBasicTypes.STRING)
					.addScalar("defectName",StandardBasicTypes.STRING)
					.setResultTransformer(transformer)
					.getResultList();
