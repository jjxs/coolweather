...
public class QmsVehicleTypeInfoController {
	...
	private final Logger log = LoggerFactory.getLogger(QmsVehicleTypeInfoController.class);
	...
	private final QmsVehicleTypeInfoService qmsVehicleTypeInfoService;

	private final QmsVehicleTypeClassRepository qmsVehicleTypeClassRepository;

	private final QmsVehicleTypeInfoServiceImpl qmsVehicleTypeInfoServiceImpl;

	@Resource
    private PageUtil pageUtil;

	public QmsVehicleTypeInfoController(QmsVehicleTypeInfoRepository qmsVehicleTypeInfoRepository,QmsVehicleTypeClassRepository qmsVehicleTypeClassRepository,
			QmsVehicleTypeInfoService qmsVehicleTypeInfoService, QmsVehicleTypeInfoServiceImpl qmsVehicleTypeInfoServiceImpl) {
	...
	@Timed
	public ResponseEntity<List<QmsVehicleTypeInfo>> getAllQmsVehicleTypeNewInfo(HttpServletRequest request,
			Pageable pageable) {
	 ...
	 * 创建车型
	 * 
	 * @param qmsVehicleTypeInfo 车型信息数据集
	 * @throws URISyntaxException
	 * @author DL0733
	 */
	@PostMapping("/qms-vehicle-type-infos/createInfo")
	@Timed
	public ResponseEntity<QmsVehicleTypeInfo> createQmsVehicleTypeInfo(
			@Valid @RequestBody QmsVehicleTypeInfo qmsVehicleTypeInfo) throws URISyntaxException {

		log.debug("REST request to save QmsVehicleTypeInfo : {}", qmsVehicleTypeInfo);

		// 判断车型ID是否为空
		if (qmsVehicleTypeInfo.getId() != null) {
			throw new BadRequestAlertException("A new qmsVehicleTypeInfo cannot already have an ID", ENTITY_NAME,
					"idexists");
		}
		QmsVehicleTypeInfo qmsVehicleTypeInfoCheckType = new QmsVehicleTypeInfo();

		qmsVehicleTypeInfoCheckType = qmsVehicleTypeInfoRepository
				.findByVehicleTypeAndFlagStatus(qmsVehicleTypeInfo.getVehicleType(), "0");
	 ...
	 * 车型维护
	 * 
	 * @param qmsVehicleTypeInfo 车型变更信息
	 * @throws URISyntaxException
	 * @author DL0733
	 */
	@PutMapping("/qms-vehicle-type-infos/updateInfo")
	@Timed
	public ResponseEntity<QmsVehicleTypeInfo> updateQmsVehicleTypeInfo(
			@Valid @RequestBody QmsVehicleTypeInfo qmsVehicleTypeInfo) throws URISyntaxException {
		...
		if (listInfo.size() != 0) {
			...
			if (dateUtil.compareByDataFormatterTo(qmsVehicleTypeInfo.getModifyTime(), listInfo.get(0).getModifyTime(),
					dataFormat)) {
				qmsVehicleTypeInfo.setModifyTime(dateUtil.getDBNowDate());
				qmsVehicleTypeInfo.setModifyUser(user.getUsername());
				// 更新车型信息
	...
	@Timed
	public Integer deleteQmsVehicleTypeInfo(@RequestBody QmsVehicleTypeInfo qmsVehicleTypeInfo) {
	...
	@Timed
	public ResponseEntity<QmsVehicleTypeClass> getQmsVehicleTypeClass(@PathVariable String id) {
     ...
     * 车型pop
     *
     * @param vehicleType 模糊查询参数
     * @param vehicleTypeName 模糊查询参数
     * @return
     * @throws URISyntaxException
     */
    @GetMapping("/qms-vehicle-type-infos/getVehicleTypePop")
    @Timed
    public ResponseEntity<List<VehicleTypeInfoDTO>> getVehicleTypePopInfo(HttpServletRequest request,Pageable pageable) throws URISyntaxException {
