...
@Service
public class QmsUnhealthyServiceImpl implements QmsUnhealthyService {

	private final Logger log = LoggerFactory.getLogger(QmsOrganizationInfoServiceImpl.class);
	...
	public QmsUnhealthyServiceImpl(EntityManagerFactory emf,QmsUnhealthyRepository qmsUnhealthyRepository) {
	...
	public Integer deleteNodeInfos(String id) {
		...
		try {
			...
			// 赋值给DTO
			organizationInfoCheck = query1.unwrap( org.hibernate.query.NativeQuery.class )
					.addScalar("id",StandardBasicTypes.LONG)
					.addScalar("parentCd",StandardBasicTypes.STRING)
					.addScalar("unhealthyCd",StandardBasicTypes.STRING)
					.addScalar("unhealthyName",StandardBasicTypes.STRING)
					.setResultTransformer(transformer)
					.getResultList();
			// 赋值给DTO
			organizationInfo = query.unwrap( org.hibernate.query.NativeQuery.class )
					.addScalar("id",StandardBasicTypes.LONG)
					.addScalar("parentCd",StandardBasicTypes.STRING)
					.addScalar("unhealthyCd",StandardBasicTypes.STRING)
					.addScalar("unhealthyName",StandardBasicTypes.STRING)
	...
	public List<QmsUnhealthy> getCheckIsUser(String id) {
		...
		try {
			...
			organizationInfoCheck = query1.unwrap( org.hibernate.query.NativeQuery.class )
					...
					.addScalar("parentCd",StandardBasicTypes.STRING)
					.addScalar("unhealthyCd",StandardBasicTypes.STRING)
					.addScalar("isUse",StandardBasicTypes.STRING)
					.setResultTransformer(transformer)
					.getResultList();
