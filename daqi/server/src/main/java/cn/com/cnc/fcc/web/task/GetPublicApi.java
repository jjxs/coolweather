package cn.com.cnc.fcc.web.task;
...
public class GetPublicApi {
	...
	private final Logger log = LoggerFactory.getLogger(GetPublicApi.class);
	...
	private static final String HST_ERROR = "Cannot execute to Task. Reason: %s";
	...
	public boolean GetToken() {
		...
		if (hstServerInfos.size() > 1) {
			throw new IllegalArgumentException(String.format(HST_ERROR, "Host Infomation is not Unique"));
		} else if (hstServerInfos.size() < 0) {
			throw new IllegalArgumentException(String.format(HST_ERROR, "Host Infomation is not Exist"));
		}
		
		return GetToken(hstServerInfos.get(0).getNodeUrl());
	}
