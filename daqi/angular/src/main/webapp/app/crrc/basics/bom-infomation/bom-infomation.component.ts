...
import { VehicleTypeInfoService } from '../vehicle-type-info';
...
export class BomInformationComponent implements OnInit, OnDestroy {
    currentAccount: any;
    ...
    getMaterielNameInfo() {
        this.msgs = [];
        if (this.materielCd !== '' && this.materielCd !== null && this.materielCd !== undefined) {
            // 取得选中数据
    ...
    */
    getVehicleClassNameInfo() {
        this.msgs = [];
        if (this.carType !== '' && this.carType !== undefined && this.carType !== null) {
    ...
    nodeSelect(event) {
        ...
        this.bomInformationService.find(event.node.data.id)
            .subscribe((organizationResponse: HttpResponse<IBomInformation>) => {
                ...
                this.rootMaterielIdInfo = this.qmsOrganizationInfos.rootMaterielId;
    ...
    nodeUnselect(event) {
        ...
        // 数据清空
        this.parentMaterielCd = 0;
        this.parentMaterielName = '';
        this.rootMaterielIdInfo = null;
        this.materielCd = '';
        this.materielName = '';
        this.productMode = '';
        this.remark = '';
    ...
    save() {
        ...
        if (this.parentMaterielCd === 0 || this.parentMaterielCd === '' || this.parentMaterielCd === undefined) {
            this.qmsOrganizationInfo.rootMaterielId = this.materielIdInfo;
        } else {
            this.qmsOrganizationInfo.rootMaterielId = this.rootMaterielIdInfo;
        }
        // 判断是新增还是编辑  1：新增 0：编辑
    ...
    onConfirm() {
        ...
        this.bomInformationService.updateCarType({ materielIdInfo: this.materielIdInfo, hidVehicleType: this.carTypeIdHidden }).subscribe(data => {
            if (data === 0) {
    ...
    openFile() {
        ...
        window.location.href = this.urldata + '/file/excel/bomInfo.xls';
    }
}
