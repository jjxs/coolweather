...
export class QmsProductionInspectionUpdateSelfComponent implements OnInit {
    ...
    // 工序质量控制点详细 错误集合
    // qualityControlDetailsShowErrorSet = new Set();
    // 错误集合
    errorList = new Array();
    ...
    // 删除按钮
    delete() {
        this.qmsProductionInspectionService.deleteCheck({pid: this.qmsProductionInspection.id}).subscribe(data => {
            if (data.body.code === "1"){
                this.messageService.add({ key: 'c', sticky: true, severity: 'shibai', summary: '确认删除', detail: '你确定要删这项任务吗？' });
            }else if(data.body.code === "2"){
                this.msgs.push({ severity: 'error', summary: '提示', detail: '当前数据的互检有数据!' });
            }else {
                this.msgs.push({ severity: 'error', summary: '提示', detail: '存在不良记录!' });
            }
        })
    }
    ...
    // 删除全部
    deleteALL() {
        this.qmsProductionInspectionService.deleteAll({pid: this.qmsProductionInspection.id}).subscribe(data => {
            if(data.body.code == "1") {
                if (this.digiFiles.length > 0) {
                    for (let a = 0; a < this.digiFiles.length; a++) {
                        const delParams = { fileId:  this.digiFiles[a].enclosureAddress };
    ...
    getMasterList(isOk) {
        ...
        this.http.get(SERVER_API_URL + 'api/qms-materiels/master', { params }).subscribe(
            data => {
                ...
                this.isOkName = this.checkStatusFormat(isOk)
            }
        );
    }

    // 检验状态Format
    checkStatusFormat(isOk) {
        let label = ""
        for (let i=0;i<this.checkStatus.length;i++){
            if (this.checkStatus[i].value === isOk){
                label = this.checkStatus[i].label
            }
        }
        return label
    }
    ...
    goPop(rowData, index) {
        if (rowData.productMode === 'M') {
            ...
            toBatch.componentInstance.materielId = rowData.materielId;
            ...
            modalRef.result.then(
                (result) => {
                    console.log(result)
                    this.qmsPartsAssemblyRelation[index].entryType = result.entryType
                    this.qmsPartsAssemblyRelation[index].hid = result.id
    ...
    uploadFlowFiles(id) {
        ...
        this.http.post(this.ipAddress + '/imgServer/upLoadAllImage', formData).subscribe(
            data => {
                console.log(data);
    ...
    async updateEnclosureDB(data) {
        ...
        for (let i = 0; i < data['retMsg'].length; i++) {
            ...
            params = {
                ...
                inspectionKbn: '2',
                enclosureAddress: this.subPath + file
            }
            try {
        ...
        for (var i = 0; i < this.qmsQualityControlDetails.length; i++) {
            ...
            if (this.qmsQualityControlDetails[i].inspectionResultDiff == '0') {
                ...
                if (this.qmsQualityControlDetails[i].upperDeviation !== null) {
                    if (Number(this.qmsQualityControlDetails[i].testValue) > Number(this.qmsQualityControlDetails[i].standard) + Number(this.qmsQualityControlDetails[i].upperDeviation)) {
                ...
                if (this.qmsQualityControlDetails[i].lowerDeviation !== null) {
                    if (Number(this.qmsQualityControlDetails[i].testValue) < Number(this.qmsQualityControlDetails[i].standard) - Number(this.qmsQualityControlDetails[i].lowerDeviation)) {
                        // this.msgs.push({ severity: 'error', summary: '提示', detail: '存在数据不符合技术要求!' });
                        this.qualityControlDetailsErrorSet.add(i)
                    }
                }
    ...
    findAll () {
        ...
        this.activatedRoute.data.subscribe(({ qmsProductionInspection }) => {
            ...
            this.getMasterList(qmsProductionInspection.isOk)

            // 获取一览数据

            // 质量检验项目
            this.qmsProductionInspectionService
            .findQmsQualityControlDetailsByTechId({ pid: this.qmsProductionInspection.id, isDetails: "0"})
            .subscribe(data => {
    ...
    save() {
        this.subPath = this.subPath + this.qmsProductionInspection.checkNumber + "/"
        // 向生产检验结果表插入数据
        ...
        // 装配check
        this.qmsProductionInspectionService.checkProductionRelation({pid: this.qmsProductionInspection.id, params: this.qmsPartsAssemblyRelation}).subscribe(data => {

            if (data.body.result === '2') {
                this.msgs.push({ severity: 'error', summary: '提示', detail: '装配重复使用!' });
                this.qualityControlDetailsErrorSet = new Set()
                this.errorList = new Array();
            }else if (data.body.result === '3') {
                this.msgs.push({ severity: 'error', summary: '提示', detail: ' 此工序已进入评审阶段，不可修改!' });
                this.qualityControlDetailsErrorSet = new Set()
                this.errorList = new Array();
                ...
                this.qmsProductionInspectionService.saveAll(params).toPromise().then(data => {
                    ...
                    if (data.body.code === '1') {
                        ...
                        this.msgs.push({ severity: 'error', summary: '提示', detail: '保存出错!' });
                        this.qualityControlDetailsErrorSet = new Set()
                        this.errorList = new Array();
                    }
                })
    ...
    check() {
        ...
        // 检验编号check
        if (this.qmsProductionInspection.checkNumber === "" || this.qmsProductionInspection.checkNumber === undefined || this.qmsProductionInspection.checkNumber === null || this.qmsProductionInspection.checkNumber.length === 0) {
        ...
        for (var i = 0; i < this.qmsQualityControlDetails.length; i++) {
            ...
            if (!this.isNumber(this.qmsQualityControlDetails[i].testValue) && this.qmsQualityControlDetails[i].testValue.toString() !=='Y' && this.qmsQualityControlDetails[i].testValue.toString() !== 'N'){
                this.msgs.push({ severity: 'error', summary: '提示', detail: '实测值必须为数字!' });
                return
            }
        }
