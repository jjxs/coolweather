...
export class QmsProductionInspectionSelfComponent implements OnInit, OnDestroy {
    ...
    check(productProcess) {
        ...
        // 检验上工序是否合格， 不合格则无法校验
        const params = {
            productionInspection : productProcess
        }

        this.qmsProductionInspectionService.chackPreProcess(params).subscribe(data => {
            
            if (data.body.code === '0') {
                this.msgs.push({ severity: 'error', summary: '提示', detail: '上工序未通过!' });
            } else if (data.body.code === '2'){
                this.msgs.push({ severity: 'error', summary: '提示', detail: '此工序已进入评审阶段，不可修改!' });
            } else{
                this.router.navigate(['/productProcessSelfCheck/qms-production-inspection-self/'+ productProcess.id +'/edit']);
            }
        })
