...
export class QmsProductionInspectionCheckComponent implements OnInit {
    ...
    // 编号离开焦点
    productBlurMar() {
        if (this.qmsProductionInspection.serialNumber!=='' && this.qmsProductionInspection.serialNumber!==null && this.qmsProductionInspection.serialNumber !== undefined) {
            this.qmsProductionInspectionService.getProduct({ materielId: this.qmsProductionInspection.materielId, productNum: this.qmsProductionInspection.serialNumber })
            .subscribe(data => {
                if (data.body === 0) {
                    this.qmsProductionInspection.serialNumber = null;
                    this.msgs.push({ severity: 'error', summary: '提示', detail: '该编号不存在!' });
                }
            });
        }
    }
    ...
    //             }else {
    //                 this.qmsProductionInspection.materielId = data.body.id;
            ...
            if (data.body.code === '1'){
                ...
                this.qmsProductionInspectionService.doBatchGeneration(this.qmsProductionInspection).subscribe(data => {
                    if (data.body.code === "1") {
                        ...
                        this.msgs.push({ severity: 'error', summary: '提示', detail: '物料和编号在任务表存在!' });
                        
                    }else if(data.body.code == "3"){
                        this.msgs.push({ severity: 'error', summary: '提示', detail: '物料和中梁号在任务表存在!' });
                    }
                    else {
                ...
                this.msgs.push({ severity: 'error', summary: '提示', detail: '物料工序不存在!' });
            }
        })
