...
export class RbacRightUpdateComponent implements OnInit {
    ...
    save() {
        ...
        if (this.rbacRight.id !== undefined) {
            ...
            this.rbacRightService.updateRight(this.rbacRight, this.menuList).subscribe(data => {
                if (data.body === 0) {
                    // 关闭画面
                    this.router.navigate(['rbacRight'], { queryParams: { savaFlag: 2 } });
                } else if (data.body === 1) {
            ...
            //从新增进入
            //供应商编码重复的Check
            this.rbacRightService.sameCheck({ samecheck: this.rbacRight.rightCode }).subscribe(data => {
                if (data.body === 1) {
                    this.msgs.push({ severity: 'error', summary: '提示', detail: '该权限信息已存在!' });
                    this.isSaving = false;
                } else {
                    this.rbacRightService.createRight(this.rbacRight, this.menuList).subscribe(data => {
                        if (data.body === 0) {
                            // 关闭画面
                            this.router.navigate(['rbacRight'], { queryParams: { savaFlag: 1 } });
                        } else {
    ...
    // 打开物料工序选择
    goToMPS() {
        this.modalService.open(BomInformationComponent1 as Component, { backdrop: 'static', keyboard: false, size: 'lg' }).result.then(
            (result) => {
                if (result !== undefined) {
                    console.log(result)
                }
