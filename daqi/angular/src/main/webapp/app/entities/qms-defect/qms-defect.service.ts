...
export class QmsDefectService {
    ...
    create(qmsDefect: IQmsDefect): Observable<EntityResponseType> {
        ...
        return this.http
            .post<IQmsDefect>(this.resourceUrl, copy, { observe: 'response' })
    ...
    update(qmsDefect: IQmsDefect): Observable<EntityResponseType> {
        const copy = this.convertDateFromClient(qmsDefect);
        return this.http
            .put<IQmsDefect>(this.resourceUrl, copy, { observe: 'response' })
            .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
    }

    find(id: number): Observable<EntityResponseType> {
        return this.http
            .get<IQmsDefect>(`${this.resourceUrl}/${id}`, { observe: 'response' })
            .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
    }
