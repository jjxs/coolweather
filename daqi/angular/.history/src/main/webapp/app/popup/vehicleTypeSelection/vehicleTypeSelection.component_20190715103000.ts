...
export class VehicleTypeSelectionComponent implements OnInit {
    ...
    // 初始化参数：一览返回数据总条数
    qmsVehicleTypeClass: IQmsVehicleTypeInfo[];
    // 初始化参数：确认返回数据
    qmsVehicleTypeClassBack: IQmsVehicleTypeInfo[];
    links: any;
    totalItems: any;
    ...
    private paginateQmsVehicleTypeClasses(data: any, headers: HttpHeaders) {
        ...
        this.qmsVehicleTypeClass = data;
        if (data.length > 0) {
            this.qmsVehicleTypeClassBack = data[0] ;
        }
    }
    ...
    selected() {
        ...
        this.msgs = [];
        // 判断是否选择数据
        if (this.qmsVehicleTypeClassBack.length === 0) {
            // 错误消息提示
            ...
            // 保存返回到上一页
            this.activeModal.close(this.qmsVehicleTypeClassBack);
        }
    }
}
