...
export class QmsProductionInspectionCheckComponent implements OnInit {
    ...
    // 编号离开焦点
    productBlurMar() {
        this.qmsProductionInspectionService.getMateriel({ materielCd: this.qmsProductionInspection.materielId, productNum: this.qmsProductionInspection.serialNumber })
            .subscribe(data => {
                if (data.body === 0) {
    ...
    materieBlurMar() {
        ...
        this.qmsProductionInspectionService.getProduct({ materielCd: this.materielCd })
            .subscribe(data => {
                if (data.body === null) {
                    this.materielCd = "";
                    this.materielName = "";
