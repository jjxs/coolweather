...
export class QmsProductionInspectionUpdateSelfComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.activatedRoute.data.subscribe(({ qmsProductionInspection }) => {
            ...
            .subscribe(data => {
                ...
                // // 检查人
                // if (this.qmsPartsAssemblyRelation.length > 0 ) {
                //     this.makeUser = this.qmsPartsAssemblyRelation[0].makeUser;
                // }
                console.log('qmsQualityControlDetails',this.qmsQualityControlDetails)
    ...
    save() {
        ...
        for (var i = 0 ;i < this.qmsQualityControlDetails.length ;i ++) {
            if (this.qmsQualityControlDetails[i] == null || this.qmsQualityControlDetails[i].standard.toString() == '') {
        ...
        // 实测值是否合格
        for (var i = 0 ;i < this.qmsQualityControlDetails.length ;i ++) {
            if (this.qmsQualityControlDetails[i].standard.toString() == '') {
                // this.msgs.push({ severity: 'error', summary: '提示', detail: '物料名称不能为空!' });
                return
            }
        }
