...
@Component({
    selector: 'jhi-self-check-details',
    templateUrl: './self-check-details.component.html',
    styleUrls: [
...
export class QmsProductionInspectionDetailsSelfComponent implements OnInit {
    // 生产检验表数据
            ...
            this.http.get(SERVER_API_URL + 'api/process-informations-infos/deleteFile', { params }).subscribe(
                data => {
                    ...
                    if (data) {
                        ...
                        this.http.get('http://localhost:8082/imgServer/deleteFile', { params: delParams }).subscribe(
                            data => {
                                console.log(data);
                                if (data) {
                                }
    ...
    viewFile(id) {
        ...
        if (imgType.indexOf(fileType) >= 0 || videoType.indexOf(fileType) >= 0 || pdfType.indexOf(fileType) >= 0) {
            ...
            this.http.get('http://localhost:8082/imgServer/viewFile', { responseType: 'blob', params }).subscribe(
                data => {
                    console.log(data);
    ...
    findQmsQualityControlDetailsByTechId () {
        ...
        .findQmsQualityControlDetailsByTechId({ pid: this.qmsProductionInspection.id, isDetails: "1"})
        .subscribe(data => {
