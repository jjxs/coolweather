...
export class QmsProductionInspectionService {
    ...
    //批量任务生成
    doBatchGeneration(qmsProductionInspection: IQmsProductionInspection): Observable<EntityResponseType> {
        // const copy = this.convertDateFromClient(qmsProductionInspection);
        return this.http
            .post<IQmsProductionInspection>('api/productProcess/doBatchGeneration', qmsProductionInspection, { observe: 'response' })
            // .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
    }
    ...
    deleteChack(req?: any): Observable<any> {
        return this.http
            .get<IQmsProductionInspection[]>('api/productProcess' +'/chackPreProcess', { params: req, observe: 'response' })
    }
