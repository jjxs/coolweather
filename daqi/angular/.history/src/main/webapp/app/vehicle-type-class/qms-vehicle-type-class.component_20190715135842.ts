...
import { RbacRoleSelectionComponent } from '../popup/rbacRoleSelection';
import { ProcessSelectionComponent } from '../popup/processSelection';
import { VehicleTypeSelectionComponent } from '../popup/vehicleTypeSelection';
...
export class QmsVehicleTypeClassComponent implements OnInit, OnDestroy {
    ...
    // 打开物料工序选择
  goToMPS() {
    this.modalService.open(VehicleTypeSelectionComponent as Component, { backdrop: 'static', keyboard: false, size: 'lg' }).result.then(
      (result) => {
        if (result !== undefined) {
            console.log(result)
        }
