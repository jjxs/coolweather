...
import { VehicleTypeClassSelectionComponent } from '../popup/vehicleTypeClassSelection/vehicleTypeClassSelection.component';
...
export class QmsVehicleTypeClassComponent implements OnInit, OnDestroy {
    ...
    // 打开物料工序选择
  goToMPS() {
    this.modalService.open(VehicleTypeClassSelectionComponent as Component, { backdrop: 'static', keyboard: false, size: 'lg' }).result.then(
      (result) => {
        if (result !== undefined) {
            console.log(result)
        }
